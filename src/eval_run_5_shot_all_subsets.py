...
for i in range(1, 9):
    ...
    ic_dataset_path = path_prefix + f"/../data/cot_ic_set_{i}.csv"
    print("loading In-context examples from: ", ic_dataset_path)
    ic_df = pd.read_csv(ic_dataset_path)

    print("IC example ids: ")
    for ic_id in ic_df["id"]:
        print(ic_id)

    ic_examples = load_in_context_examples(path_or_df=ic_df, conversion_dict=label_to_answer)
